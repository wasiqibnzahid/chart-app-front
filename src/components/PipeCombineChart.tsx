...
const PipeCombineChart: React.FC<AverageChartProps> = ({
	...
	const dataToUse = useMemo(() => {
		...
		return mainDataUse;
	}, [data, selectedDropdown, quarterVal, selectedYear, showAllData, dataFromDate, showDataFromDate]);
	...
	const dropDownDates = useMemo(() => {
		return dataToUse
			?.filter((data) => data?.name === "TV Azteca")[0]
			?.data?.map((d) => d?.x);
	}, [dataToUse]);

	useEffect(() => {
		if(dropDownDates && dropDownDates.length > 0 && dateFilter === "") setDateFilter(dropDownDates[0])
