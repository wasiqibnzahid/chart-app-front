...
const GROUPS = {
    "TV Azteca Companies": ["deportes", "7", "noticias", "adn40", "Amastv", "Uno"],
    Competitors: ["Milenio", "El Heraldo", "El Universal", "Televisa", "Terra", "AS",
        "Infobae", "NY Times"],
};
...
const FORM_FACTOR_COLORS = {
    phone: "#00FFFF", // Bright Cyan
    desktop: "#FFA500", // Bright Orange
};
...
const PerformanceMapVertical = () => {
    ...
    const [selectedFormFactors, setSelectedFormFactors] = useState(["phone", "desktop"]);
    ...
    useEffect(() => {
        const csvUrl = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQkary38OmPhvyBG9ZgDkJbcZmP4Q0_qYVceNyQPDcrr0HERgtq2C46ImlSGnFL9Etfw4PaC9y0xcpL/pub?output=csv";
        Papa.parse(csvUrl, {
            download: true,
            ...
            complete: (results) => {
                try {
                    const parsedData = results.data.map((row, index) => ({
                        ...
                        formFactor: row["Form Factor"] ? row["Form Factor"].trim().toLowerCase() :
                            "",
                    ...
                    setData(validData);
                    // Remove automatic selection of the most recent week
                    // Preselect the most recent week is removed to show all time ranges by default
                    setIsLoading(false);
                } catch (err) {
                                    ...
                                    <PopoverBody>
                                        <VStack align="start" spacing={2}>
                                            ...
                                            <Box>
                                                ...
                                                >
                                                    <HStack spacing={2}>
                                                        <Checkbox value="phone" color="white">
                                                            Phone
                                                        </Checkbox>
                                                        <Checkbox value="desktop" color="white">
                                                            Desktop
                                                        </Checkbox>
                                                    </HStack>
                                                </CheckboxGroup>
